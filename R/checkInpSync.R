...
checkInpSync <- function(inp_sync){
	# speed of sound stuff
	if(inp_sync$dat_tmb_sync$ss_data_what != "data"){cat("WARNING: getSyncModel() will estimate speed of sound. It is strongly advised to use data instead!\n")}
	...
	stopifnot(inp_sync$dat_tmb_sync$fixed_hydros_vec[inp_sync$dat_tmb_sync$tk ] == 1)
	...
	if(min(sync_coverage$N) <= 5) {cat("ERROR: At least one hydro x offset_idx combination has less than 5 observations. This hydro cannot be synced in that period!\n")}
	stopifnot(min(sync_coverage$N) > 5)
	if(min(sync_coverage$N) < 10) {
	...
	} else if(min(sync_coverage$N) < 50) {
		cat("WARNING: At least one hydro has less than 50 pings in an offset_idx - try getSyncCoverage(inp_sync, plot=TRUE) for visual\n and rerun getInpSync() with increased keep_rate\n")
	}
}

#' Quick overview to check if all hydros have enough data within each offset period.
#'
#' @inheritparams checkInpSyn
#' @export
getSyncCoverage <- function(inp_sync, plot=FALSE){
	...
	if(plot){
		p <- ggplot(sync_coverage) + geom_point(aes(offset_idx, N)) 
